...
public class CrochetApp {
    ...
    private Color selectColor() throws Exception {
        ...
        int colorChoice = 0; // forces entry into loop
        boolean runLoop = true;
        Color output = null;
        int loopCounter = 0;

        while (runLoop) {
            System.out.println("What colour would you like? (input rgb)");
            colorChoice = input.nextInt();
            if (colorChoice > (255 * 256 + 255) * 256 + 255) {
                output = new Color(colorChoice);
                runLoop = false;
            }
